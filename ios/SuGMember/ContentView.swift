...
struct ContentView: View {
    ...
    @StateObject private var bodyStore = MemberWebViewStore(url: URL(string: base + "apps/body/?native=ios&v=27.66")!)
    @StateObject private var questStore = MemberWebViewStore(url: URL(string: base + "shrine-quest-v26.5.206.html?embedded=1&native=ios&v=27.66")!)
    @StateObject private var walkStore = MemberWebViewStore(url: URL(string: base + "walk-quest.html?embedded=1&native=ios&v=27.66")!)
    @StateObject private var cardStore = MemberWebViewStore(url: URL(string: base + "?entry=member&hub=1&membercard=1&native=ios&v=27.66")!)
