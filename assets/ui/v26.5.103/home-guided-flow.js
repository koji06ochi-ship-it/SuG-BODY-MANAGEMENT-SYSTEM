...
function findTrainingTarget(){const cards=visibleCards();return cards.find(c=>/今日のメニュー|トレーニングメニュー|おすすめ.*メニュー|種目|SET|セット|NEXT LOAD/i.test(c.textContent||''))||null}
function goTraining(){clearFocus();const target=findTrainingTarget();if(target){target.classList.add('sug-active-step');const b=document.createElement('div');b.className='sug-step-badge';b.textContent='④ 今日やるトレーニング';target.prepend(b);requestAnimationFrame(()=>scrollTo(0,Math.max(0,target.getBoundingClientRect().top+scrollY-105)));return}try{if(typeof window.openTab==='function')window.openTab('training');else document.querySelector('.tab[data-tab="training"],.tab[data-tab="train"]')?.click()}catch(e){}setTimeout(()=>{const p=document.querySelector('.panel.active');if(p)scrollTo(0,Math.max(0,p.getBoundingClientRect().top+scrollY-95))},120)}
