...
.sug-dim-control,.sug-dim-block{opacity:.22!important;pointer-events:none!important}
...
function resetForNewDay(){if(contextIsToday())return false;const t=todayControls();if(!t)return false;ensurePlaceholder(t.appetite,'選択してください');ensurePlaceholder(t.day,'選択してください');t.appetite.value='';t.day.value='';return true}
...
function runRecalc(){const t=todayControls();if(!t)return;clearFocus();try{t.recalc?.click()}catch(e){}setTimeout(nextSection,450)}
function startToday(){hideHub();openSmart();setTimeout(()=>{if(contextIsToday())runRecalc();else{resetForNewDay();focusTodayStep(0)}},160)}
function onChange(e){const t=todayControls();if(!t)return;if(e.target===t.appetite){if(!t.appetite.value)return;setTimeout(()=>focusTodayStep(1),40)}else if(e.target===t.day){if(!t.day.value)return;setTimeout(runRecalc,40)}}
function closeIdealAndAdvance(){try{window.closeIdealVision?.()}catch(e){}sessionStorage.removeItem('sug_waiting_ideal');document.getElementById('sug-guided-flow')?.remove();setTimeout(startToday,120)}
...
window.__SUG_GUIDED_HOME_VERSION__='26.5.120';
})();