...
function hasMeaningfulValue(el){if(!el)return false;const v=String(el.value??'').trim();return v!==''&&v!=='未選択'&&v!=='選択してください'}
function autoRunRecalc(){const t=todayControls();if(!t||!t.recalc)return;const {selects,recalc,card}=t;if(selects.length<2)return;if(!hasMeaningfulValue(selects[0])||!hasMeaningfulValue(selects[1]))return;if(recalc.dataset.sugAutoRunning==='1')return;recalc.dataset.sugAutoRunning='1';focusCard(card,recalc,'入力完了。自動で今日の内容を計算中…');setTimeout(()=>{try{recalc.click()}catch(e){}setTimeout(()=>{recalc.dataset.sugAutoRunning='0';clearFocus();focusNextVisibleInput()},450)},180)}
function fieldWatcher(e){const t=todayControls();if(!t)return;if(e.target===t.selects[0]){setTimeout(()=>focusTodayStep(1),80);setTimeout(autoRunRecalc,140);return}if(e.target===t.selects[1]){setTimeout(autoRunRecalc,100);return}}
function cardInteractionWatcher(e){const t=todayControls();if(!t||!t.card.contains(e.target))return;setTimeout(autoRunRecalc,160)}
...
document.addEventListener('click',idealSaveWatcher,true);document.addEventListener('change',fieldWatcher,true);document.addEventListener('input',fieldWatcher,true);document.addEventListener('click',cardInteractionWatcher,true);document.addEventListener('click',clickWatcher,true);document.addEventListener('error',e=>{if(e.target?.tagName==='IMG')hideBrokenImage(e.target)},true);
...
window.focusSugTodayStep=focusTodayStep;window.__SUG_GUIDED_HOME_VERSION__='26.5.112';
})();