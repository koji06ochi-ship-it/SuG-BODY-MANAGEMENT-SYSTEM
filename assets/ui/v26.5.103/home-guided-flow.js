...
function focusTodayStep(step=0){const t=todayControls();if(!t||!t.card){focusNextVisibleInput();return}const {card,selects,recalc}=t;
 ...
 if(recalc){focusCard(card,recalc,'③ 最後にここを押す');selects.forEach(x=>{x.classList.add('sug-dim-control');blockFor(x)?.classList.add('sug-dim-block')});return}
...
function mount(){if(document.getElementById('sug-guided-flow')||hasIdeal())return;const box=document.createElement('section');box.id='sug-guided-flow';box.innerHTML=`<div class="eyebrow">TODAY FLOW</div><h2>理想の身体を選ぶ</h2><p>体型を決めたら、×を押さずに自動で次へ進みます。</p><button type="button">理想を選ぶ →</button>`;box.querySelector('button').onclick=openIdeal;const header=document.querySelector('header');if(header?.parentNode)header.insertAdjacentElement('afterend',box);else(document.querySelector('main')||document.body).prepend(box)}
...
function fieldWatcher(e){const t=todayControls();if(!t)return;if(e.target===t.selects[0]){setTimeout(()=>focusTodayStep(1),100);return}if(e.target===t.selects[1]){setTimeout(()=>focusTodayStep(2),100);return}}
function clickWatcher(e){const t=todayControls();if(!t)return;if(t.recalc&&e.target.closest?.('button')===t.recalc){clearFocus();setTimeout(focusNextVisibleInput,280)}}
...
window.focusSugTodayStep=focusTodayStep;window.__SUG_GUIDED_HOME_VERSION__='26.5.110';
})();