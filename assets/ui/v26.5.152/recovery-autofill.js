...
function boot(){retry();window.addEventListener('sug:recovery-mounted',retry);window.addEventListener('pageshow',()=>setTimeout(prefill,120));document.addEventListener('click',e=>{if(e.target?.closest?.('#sug149Apply,.sug149-next,#smart button'))setTimeout(prefill,180)},true)}
