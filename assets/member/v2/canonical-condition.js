...
function cloudConfig(){
  ...
  for(const scope of scopes){
    try{
      const c=scope.SUG_CLOUD_CONFIG;
      if(c?.url&&c?.publishableKey)return c;
    }catch(_){}
  }
...
function latestCare(data,recovery){
  ...
  for(const c of (Array.isArray(data?.selfCare)?data.selfCare:[])){
    if(c?.result==='medical'||c?.result==='stop'){
      signals.push({c,pain:Number(c.painAfter??c.painBefore??0),at:c.savedAt||c.date,signal:'immediate'});
    }else if(c?.result==='selfcare'&&c?.painAfter!=null){
      signals.push({c,pain:Number(c.painAfter),at:c.savedAt||c.date,signal:'immediate'});
    }
...
function conditionFromData(data){
  ...
  return {
    recovery:r?{
      sleep_minutes:Number(r.sleep||0)*60,
...
async function read(auth){
  ...
  if(!cfg?.url||!cfg?.publishableKey)return {ok:false,error:'CLOUD_CONFIG_MISSING'};
  try{
    ...
    if(!row?.data)return {ok:false,error:'MEMBER_DATA_MISSING'};
