(()=>{
  ...
  const SPRITE='./assets/quest/quest-v213-characters-sprite.webp?v='+VERSION;
  ...
  .v213Char.on{border-color:#d1a54d;box-shadow:0 0 0 1px #d1a54d44}.v213Avatar{display:block;width:78px;height:78px;border-radius:10px;background-image:url('${SPRITE}');background-size:500% 400%;background-repeat:no-repeat;background-color:#21190e}
  ...
  function setAvatar(el,i){el.style.backgroundPosition=pos(i)}
  function mount(doc){
    ...
    loadManifest().then(data=>{
      ...
      box.innerHTML='<h3>S.u.G QUEST CHARACTERS</h3><p>街探索・トレーニング・回復・歴史ルートで登場するキャラ</p><div class="v213Strip"></div><div class="v213Dialogue"><div class="v213Avatar"></div><div><b></b><span></span></div></div>';
      ...
      data.characters.forEach((c,i)=>{
        const b=doc.createElement('button');b.className='v213Char';b.dataset.character=c.id;b.innerHTML='<div class="v213Avatar"></div><b></b><small></small>';setAvatar(b.querySelector('.v213Avatar'),i);b.querySelector('b').textContent=c.name;b.querySelector('small').textContent=c.role;
        b.onclick=()=>{strip.querySelectorAll('.v213Char').forEach(x=>x.classList.remove('on'));b.classList.add('on');dlg.classList.add('on');setAvatar(dlgAvatar,i);dlgName.textContent=c.name;dlgLine.textContent=lines[c.role]||'QUESTで登場するキャラクター。'};
