(()=>{'use strict';
const KEY='sug_rewards_v1',COST_KEY='sug_reward_costs_v1';
const BASE=[
{id:'protein',name:'プロテイン',points:5000,costYen:null,kind:'BODY',note:'継続記録の交換特典。実費設定後に交換可能。'},
{id:'meal1000',name:'提携飲食店 1,000円OFF',points:7000,costYen:1000,kind:'FOOD',note:'提携店・対象メニュー・利用条件を確認して発行。'},
{id:'powergrip',name:'パワーグリップ',points:15000,costYen:null,kind:'BODY',note:'中長期継続向け。商品原価設定後に交換可能。'},
{id:'bracebelt',name:'腹圧ベルト',points:25000,costYen:null,kind:'BODY',note:'長期継続向け。商品原価設定後に交換可能。'}
];
...
function catalog(){const c=costs();return BASE.map(x=>{const override=Number(c[x.id]);const cost=Number.isFinite(override)&&override>0?Math.round(override):x.costYen;return{...x,costYen:cost,costReady:Number.isFinite(Number(cost))&&Number(cost)>0}})}
