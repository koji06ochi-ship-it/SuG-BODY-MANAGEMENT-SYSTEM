(()=>{'use strict';
const V='0.3.0',HEALTH='sug_native_health_v1',DAILY='sug_body_lite_daily_v1',PREF='sug_body_lite_pref_v1',QUEST='sug_walk_quest_v1',WATCH='sug_body_lite_watch_v1',LIFE='sug_body_lite_lifestyle_v1',FRAG='sug_body_lite_fragments_v1',PHOTO_DB='sug_body_lite_photos_v1',PHOTO_STORE='photos';
...
const RANKS=[
{min:0,title:'星を見上げる旅人',story:'今日の一歩が、物語の始まり。'},
{min:3.5,title:'継続の勇者',story:'続けた日数が、自分の力になる。'},
{min:5.5,title:'神宝の守り手',story:'身体を知り、行動を選べる者。'},
{min:7.5,title:'星導の武者',story:'星を標に、己の道を進む。'},
{min:9,title:'十種神宝の継承者',story:'十の神宝を整え、自らの物語を歩む者。'}
];
...
function renderLifestyle(){if($('#stretchMinutes'))$('#stretchMinutes').value=lifestyle.stretchMinutes||'';if($('#waterL'))$('#waterL').value=lifestyle.waterL||'';if($('#nutrition'))$('#nutrition').value=lifestyle.nutrition||''}
function renderTreasures(){const {states,count}=treasureState(),box=$('#treasureGrid');if(box)box.innerHTML=states.map(t=>`<div class="treasure ${t.on?'on':''}"><div class="relic">${t.icon}</div><div><b>${t.name}</b><small>${t.label}｜${t.desc}</small><div class="ok">${t.on?'● 本日装備':'○ まだ眠っている'}</div></div></div>`).join('');if($('#treasureCount'))$('#treasureCount').textContent=`${count}/10`;if($('#treasureProgress'))$('#treasureProgress').style.width=`${count*10}%`;const level=Math.max(1,Math.min(5,Math.ceil(count/2)));if($('#todayStars'))$('#todayStars').textContent=stars(level);if($('#todayRank'))$('#todayRank').textContent=count===10?'十種神宝、すべて装備':`${count}つの神宝が目覚めています`;const missing=states.filter(x=>!x.on).slice(0,3).map(x=>x.label).join('・');if($('#todayHint'))$('#todayHint').textContent=count===10?'今日は十の神宝がすべてそろいました。':missing?`次に狙う：${missing}`:'今日の行動で神宝が光ります。'}
...
function renderFragment(){const all=j(FRAG,{}),value=all[monthKey()]||'';if($('#fragmentInput')&&document.activeElement!==$('#fragmentInput'))$('#fragmentInput').value=value;if($('#fragmentStatus'))$('#fragmentStatus').textContent=value?'今月の人生の断片を保存済み':'まだ今月の断片はありません'}
function summary(){const c=monthStats(monthKey()),rank=rankFor(c.treasureAvg??0),frag=j(FRAG,{})[monthKey()]||'--';return [`S.u.G BODY Lite｜${monthKey()}`,`称号: ${rank.title} ${stars(rank.level)}`,`平均神宝: ${c.treasureAvg==null?'--':c.treasureAvg.toFixed(1)} / 10`,`平均歩数: ${c.steps==null?'--':Math.round(c.steps).toLocaleString()} 歩/日`,`QUEST参加日: ${c.questDays}日`,`外出日: ${c.outsideDays}日`,`人生の断片: ${frag}`].join('\n')}
function renderAll(){renderHealth();renderLifestyle();renderTreasures();renderQuest();renderMode();renderCondition();renderMonth();renderFragment();renderPhotos()}
...
function boot(){
  $('#mode').onchange=e=>{pref.mode=e.target.value;localStorage.setItem(PREF,JSON.stringify(pref));renderMode();saveDaily();renderAll()};
  ...
  $$('.scale button').forEach(b=>b.onclick=()=>{condition[b.dataset.key]=+b.dataset.v;renderCondition();saveDaily();renderAll()});
  ...
  window.addEventListener('storage',e=>{if([HEALTH,QUEST,DAILY,PREF,WATCH,LIFE,FRAG].includes(e.key)){if(e.key===HEALTH)health=j(HEALTH,{});if(e.key===PREF)pref=j(PREF,pref);if(e.key===WATCH)watchState=j(WATCH,watchState);if(e.key===LIFE){lifeAll=j(LIFE,{});lifestyle={stretchMinutes:0,waterL:0,nutrition:'',...(lifeAll[today()]||{})}}questChanged();renderAll()}});
...
window.SuGBodyLite={version:V,receiveNative:applyHealth,render:renderAll,summary,record,quest,latestLocation,treasureState,rankFor,startWatch,stopWatch};
