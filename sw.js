const SW_VERSION = 'sug-v26.5.44-onboarding-flow';
...
self.addEventListener('fetch', event => {
 ...
 if(req.mode==='navigate'){
  event.respondWith((async()=>{try{const res=await fetch(req,{cache:'no-store'});const type=res.headers.get('content-type')||'';if(!type.includes('text/html'))return res;let html=await res.text();const tags=[
   '<script src="assets/member-home/v26.5.41/engine.js?v=26.5.44"></script>',
   '<script src="assets/member-home/v26.5.42/login-render.js?v=26.5.44"></script>',
   '<script src="assets/member-home/v26.5.44/onboarding.js?v=26.5.44"></script>'
